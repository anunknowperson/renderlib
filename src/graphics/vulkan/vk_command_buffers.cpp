...
void CommandBuffers::init_commands(VulkanEngine* vk_engine) {
    ...
    const VkCommandPoolCreateInfo commandPoolInfo =
            vkinit::command_pool_create_info(
                    vk_engine->getQueueIndex(),
                    VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);

    for (auto& _frame : vk_engine->command_buffers_container._frames) {
        VkCommandPool commandPool;
        VK_CHECK(vkCreateCommandPool(vk_engine->getLogicalDevice(), &commandPoolInfo,
                                     nullptr,
                                     &commandPool));

        _frame._commandPool = std::make_unique<VulkanCommandPool>(vk_engine->getLogicalDevice(), commandPool);

        // allocate the default command buffer that we will use for rendering
        VkCommandBufferAllocateInfo cmdAllocInfo =
                vkinit::command_buffer_allocate_info(_frame._commandPool->get(), 1);

        VK_CHECK(vkAllocateCommandBuffers(vk_engine->getLogicalDevice(), &cmdAllocInfo,
                                          &_frame._mainCommandBuffer));
    }

    VkCommandPool immCommandPool;
    VK_CHECK(vkCreateCommandPool(vk_engine->getLogicalDevice(), &commandPoolInfo, nullptr,
                                 &immCommandPool));

    vk_engine->command_buffers_container._immCommandPool = std::make_unique<VulkanCommandPool>(vk_engine->getLogicalDevice(), immCommandPool);
...
void CommandBuffers::immediate_submit(
    ...
    VK_CHECK(vkResetFences(vk_engine->getLogicalDevice(), 1, vk_engine->command_buffers_container._immFence->getPtr()));
    ...
    VK_CHECK(vkQueueSubmit2(vk_engine->getQueue(), 1, &submit,
                            vk_engine->command_buffers_container._immFence->get()));

    VK_CHECK(vkWaitForFences(vk_engine->getLogicalDevice(), 1, vk_engine->command_buffers_container._immFence->getPtr(), true,
                             9999999999));
}
