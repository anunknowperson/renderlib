...
void VulkanEngine::initDefaultData() {
    ...
    white_image =
            createImage(&white, VkExtent3D{1, 1, 1}, VK_FORMAT_R8G8B8A8_UNORM,
                         VK_IMAGE_USAGE_SAMPLED_BIT);

    const uint32_t grey = glm::packUnorm4x8(glm::vec4(0.66f, 0.66f, 0.66f, 1));
    grey_image =
            createImage(&grey, VkExtent3D{1, 1, 1}, VK_FORMAT_R8G8B8A8_UNORM,
                         VK_IMAGE_USAGE_SAMPLED_BIT);

    const uint32_t black = glm::packUnorm4x8(glm::vec4(0, 0, 0, 0));
    black_image =
            createImage(&black, VkExtent3D{1, 1, 1}, VK_FORMAT_R8G8B8A8_UNORM,
                         VK_IMAGE_USAGE_SAMPLED_BIT);
    ...
    const AllocatedBuffer material_constants = createBuffer(
...
void VulkanEngine::initCommands() {
    ...
    for (auto& _frame : frames) {
        ...
        VkCommandBufferAllocateInfo cmd_alloc_info =
                vkinit::commandBufferAllocateInfo(_frame.command_pool, 1);
...
void VulkanEngine::initSyncStructures() {
    ...
    const VkSemaphoreCreateInfo semaphore_create_info = vkinit::semaphoreCreateInfo();
...
void VulkanEngine::initSwapchain() {
    ...
    const VkExtent3D draw_image_extent = {window_extent.width,
    ...
    const VkImageCreateInfo rimg_info = vkinit::imageCreateInfo(
    ...
    rimg_alloc_info.requiredFlags = static_cast<VkMemoryPropertyFlags>(
    ...
    const VkImageViewCreateInfo rview_info = vkinit::imageviewCreateInfo(
...
void VulkanEngine::cleanup() {
    if (is_initialized) {
        ...
        for (const auto& _frame : frames) {
            ...
            vkDestroySemaphore(device, _frame.swapchain_semaphore, nullptr);
        }
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices,
    ...
    const AllocatedBuffer staging = createBuffer(
    ...
    main_deletion_queue.pushFunction(
            [=, this] { destroyBuffer(new_surface.vertex_buffer); });
    main_deletion_queue.pushFunction(
            [=, this] { destroyBuffer(new_surface.index_buffer); });
...
void VulkanEngine::drawBackground(VkCommandBuffer cmd) const {
    ...
    vkCmdDispatch(
            cmd, static_cast<uint32_t>(std::ceil(draw_extent.width / 16.0)),
...
void VulkanEngine::drawImgui(VkCommandBuffer cmd,
    ...
    const VkRenderingAttachmentInfo color_attachment = vkinit::attachmentInfo(
...
void VulkanEngine::drawGeometry(VkCommandBuffer cmd) {
    ...
    *scene_uniform_data = scene_data;
    ...
    single_image_writer.writeImage(0, error_checkerboard_image.image_view,
                                   default_sampler_nearest,
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                   VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    single_image_writer.updateSet(device, image_set);
    ...
    for (const auto& [indexCount, firstIndex, indexBuffer, material, transform,
                      vertexBufferAddress] : main_draw_context.opaque_surfaces) {
        ...
        pushConstants.vertex_buffer = vertexBufferAddress;
        pushConstants.world_matrix = transform;
...
void VulkanEngine::draw() {
    ...
    const VkSemaphoreSubmitInfo wait_info = vkinit::semaphoreSubmitInfo(
...
void VulkanEngine::immediateSubmit(
    ...
    const VkCommandBuffer cmd = imm_command_buffer;
    ...
    const VkCommandBufferSubmitInfo cmd_info =
            vkinit::commandBufferSubmitInfo(cmd);
    const VkSubmitInfo2 submit =
            vkinit::submitInfo(&cmd_info, nullptr, nullptr);
...
AllocatedImage VulkanEngine::createImage(const void* data, VkExtent3D size,
                                          VkFormat format,
                                          VkImageUsageFlags usage,
                                          bool mipmapped) const {
    const size_t data_size = size.depth * size.width * size.height * 4;
    const AllocatedBuffer upload_buffer =
...
void MeshNode::draw(const glm::mat4& top_matrix, DrawContext& ctx) {
    ...
    for (auto& [startIndex, count, material] : mesh->surfaces) {
        RenderObject def{};
        def.index_count = count;
        def.first_index = startIndex;
...
void VulkanEngine::updateScene() {
    ...
    const glm::mat4 view = main_camera->getViewMatrix();
    ...
    for (const auto& [key, mesh] : meshes) {
        const std::shared_ptr<LoadedGLTF> loaded_mesh = mesh;
        loaded_mesh->draw(transforms[key], main_draw_context);
    }
}

int64_t VulkanEngine::registerMesh(const std::string& file_path) {
    ...
    const std::string structure_path = {std::string(ASSETS_DIR) + file_path};
    const auto structure_file = loadGltf(this, structure_path);
