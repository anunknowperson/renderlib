...
void CommandBuffers::init_commands(VulkanEngine* vk_engine) {
    ...
    for (auto& _frame : vk_engine->command_buffers_container._frames) {
        ...
        VK_CHECK(vkCreateCommandPool(vk_engine->getRawDevice(), &commandPoolInfo,
                                     nullptr,
                                     &commandPool));

        _frame._commandPool = std::make_unique<VulkanCommandPool>(vk_engine->getRawDevice(), commandPool);

        // allocate the default command buffer that we will use for rendering
        VkCommandBufferAllocateInfo cmdAllocInfo =
                vkinit::command_buffer_allocate_info(_frame._commandPool->get(), 1);

        VK_CHECK(vkAllocateCommandBuffers(vk_engine->getRawDevice(), &cmdAllocInfo,
                                          &_frame._mainCommandBuffer));
    }

    VkCommandPool immCommandPool;
    VK_CHECK(vkCreateCommandPool(vk_engine->getRawDevice(), &commandPoolInfo, nullptr,
                                 &immCommandPool));

    vk_engine->command_buffers_container._immCommandPool = std::make_unique<VulkanCommandPool>(vk_engine->getRawDevice(), immCommandPool);
...
void CommandBuffers::immediate_submit(
    ...
    VK_CHECK(vkResetFences(vk_engine->getRawDevice(), 1, vk_engine->command_buffers_container._immFence->getPtr()));
    ...
    VK_CHECK(vkWaitForFences(vk_engine->getRawDevice(), 1, vk_engine->command_buffers_container._immFence->getPtr(), true,
                             9999999999));
}
