		...
		void* pUserData)
{
	...
	switch (messageType)
	{
	case VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT:type = "General";

		break;

	case VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT:type = "Validation";

		break;

	case VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT:type = "Performance";
...
void VulkanEngine::init_default_data()
{
	...
	//write the buffer
	auto* sceneUniformData = (GLTFMetallic_Roughness::MaterialConstants*)materialConstants.allocation->GetMappedData();
	...
	defaultData = metalRoughMaterial
			.write_material(_device, MaterialPass::MainColor, materialResources, globalDescriptorAllocator);
}
...
AllocatedBuffer
VulkanEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage) const
{
	// allocate buffer
	...
	// allocate the buffer
	VK_CHECK(vmaCreateBuffer(_allocator, &bufferInfo, &vmallocinfo, &newBuffer.buffer, &newBuffer.allocation,
			&newBuffer.info));
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
	...
	VkBufferDeviceAddressInfo deviceAddressInfo{.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO, .buffer = newSurface
			.vertexBuffer.buffer};
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd)
{
	...
	*sceneUniformData = sceneData;
	...
	VkDescriptorSet globalDescriptor = get_current_frame()._frameDescriptors
			.allocate(_device, _gpuSceneDataDescriptorLayout);
	...
	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.trianglePipeline);
	...
	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.meshPipelineLayout, 0, 1, &imageSet, 0, nullptr);
...
AllocatedImage
VulkanEngine::create_image(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped) const
{
...
AllocatedImage
VulkanEngine::create_image(void* data, VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
...
MaterialInstance
GLTFMetallic_Roughness::write_material(VkDevice device, MaterialPass pass, const MaterialResources& resources,
...
void VulkanEngine::update_scene()
{
	...
	glm::mat4 projection = glm::perspective(glm::radians(70.f), (float)_windowExtent.width/(float)_windowExtent.height,
			0.1f, 10000.f);
